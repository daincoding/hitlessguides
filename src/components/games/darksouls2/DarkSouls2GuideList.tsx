...
const DarkSouls2GuideList: React.FC<Props> = ({
  selectedRun,
  selectedGame,
}) => {
  ...
  useEffect(() => {
    ...
    fetch("/gameguides/darksouls2.json")
    ...
    <div>
      ...
      >
        {filteredGuides.map((guide) => (
          <Ds2Card key={guide.id} guide={guide} />
        ))}
      </SimpleGrid>
    </div>
  );
...
export default DarkSouls2GuideList;
