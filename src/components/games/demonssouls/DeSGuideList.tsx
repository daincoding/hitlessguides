...
const DeSGuideList: React.FC<Props> = ({ selectedRun, selectedGame }) => {
  const [guides, setGuides] = useState<Guide[]>([]); // State to hold the guides list

  useEffect(() => {
    // Fetch the local JSON file
    fetch("/gameguides/demonssouls.json")
      ...
      {filteredGuides.length > 0 ? (
        ...
        >
          {filteredGuides.map((guide) => (
            <DeSCard key={guide.id} guide={guide} />
          ))}
        </SimpleGrid>
      ) : (
